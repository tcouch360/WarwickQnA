...
from flask import render_template, redirect, url_for, g, Blueprint
...
def showQuestion(question_id):
    ...
    if g.user.is_authenticated and len(Answer.query.filter_by(question_id=question_id, user_id=g.user.user_id).all()):
        enabled = False
    return render_template('mod_question/showQuestion.html', title=question[0].title, questions=question,
                           answers=answers, enabled=enabled, comments=comments)
...
def addQuestion():
    ...
    if form.validate_on_submit():
        print(form.tag.data)
        code = "\n" + form.code.data
        question = Question(title=form.title.data, body=form.body.data, code=code, author=g.user,
                            timestamp=datetime.utcnow(), answered=0)
...
def unansweredQuestions(page=1):
    ...
    questions = Question.query.filter(Question.answered == 0).order_by(Question.timestamp.desc()).paginate(page,
                                                                                                           app.config[
                                                                                                               'POSTS_PER_PAGE'],
                                                                                                           False)
        ...
        if g.user.is_authenticated:
            if votetype == 1:
                ...
                if contenttype == 1:
                    ...
                    ans = len(
                        Upvote.query.filter(and_(Upvote.user_id == g.user.user_id, Upvote.question_id == pid)).all())
                elif contenttype == 2:
                    """Upvote on an answer"""
                    ans = len(
                        Upvote.query.filter(and_(Upvote.user_id == g.user.user_id, Upvote.answer_id == pid)).all())
                else:
                    """Upvote on a comment"""
                    ans = len(
                        Upvote.query.filter(and_(Upvote.user_id == g.user.user_id, Upvote.comment_id == pid)).all())
            else:
                """Votetype 2->Downvote"""
                if contenttype == 1:
                    """Downvote on a question"""
                    ans = len(Downvote.query.filter(
                        and_(Downvote.user_id == g.user.user_id, Downvote.question_id == pid)).all())
                elif contenttype == 2:
                    """Downvote on an answer"""
                    ans = len(Downvote.query.filter(
                        and_(Downvote.user_id == g.user.user_id, Downvote.answer_id == pid)).all())
                else:
                    """Downvote on a comment"""
                    ans = len(Downvote.query.filter(
                        and_(Downvote.user_id == g.user.user_id, Downvote.comment_id == pid)).all())
            print(ans)
        if ans >= 1:
            return 0
        else:
