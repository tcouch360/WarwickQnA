...
#from . import mod_comment
mod_comment = Blueprint('mod_comment', __name__)
"""
	Adds a comment to a given question in comment database
"""

@mod_comment.route('/addQuestionComment', methods = ['GET', 'POST'])
def addQuestionComment():
	if request.method == 'POST':
		question = Question.query.filter_by(question_id = request.form['question_id']).first()
		try:
			comment = Comment(author = g.user, question = question, body = request.form['body'])
			db.session.add(comment)
			db.session.commit()
		except:
			print ('Could not add Comment')
		print (comment.body)
		return jsonify({'username': comment.author.username ,'avatar': comment.author.avatar(120), 'comment_id': comment.comment_id, 'body': comment.body})
		#return url_for('mod_question.showQuestion', question_id = request.form['question_id'])

"""Add comment to a given answer in comment database"""
@mod_comment.route('/addAnswerComment', methods = ['GET', 'POST'])
def addAnswerComment():
	if request.method == 'POST':
		...
		try:
			comment = Comment(body = request.form['body'], author = g.user, answer = answer)
			db.session.add(comment)
			db.session.commit()
		except:
			print ("Could not add Comment")
		#return url_for('mod_question.showQuestion', question_id = answer.question_id)
		return jsonify({'username': comment.author.username ,'avatar': comment.author.avatar(120), 'comment_id': comment.comment_id, 'body': comment.body})
