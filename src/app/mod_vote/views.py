...
def questionDownvote(question_id):
    """This function does Upvote on a particular question in database"""
    question = Question.query.filter_by(question_id=question_id).first()
    if len(Downvote.query.filter(
            and_(Downvote.user_id == g.user.user_id, Downvote.question_id == question_id)).all()) == 0:
        try:
...
def commentDownvote(comment_id):
    ...
    if (comment.question_id):
        ...
        question_id = answer.question_id
    if len(Downvote.query.filter(
            and_(Downvote.user_id == g.user.user_id, Downvote.comment_id == comment_id)).all()) == 0:
        try:
